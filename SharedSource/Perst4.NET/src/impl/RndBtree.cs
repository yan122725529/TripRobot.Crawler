namespace Perst.Impl
{
    ...
    [Serializable]    
#if USE_GENERICS
    internal class RndBtree<K,V>:PersistentCollection<V>, Index<K,V> where V:class
#else
    internal class RndBtree:PersistentCollection, Index
#endif
    {
        ...
        internal abstract class BtreePage:Persistent
        {
            ...
            internal const int BTREE_PAGE_SIZE = Page.pageSize - ObjectHeader.Sizeof - 4 * 4;
            ...
            internal object getAt(int i, int height) 
            { 
                if (--height == 0) 
                {
                    return items[i];
                ...
                { 
                    ...
                    return ((BtreePage)items[j]).getAt(i, height);
                }
            }

            internal int indexOf(Key key, int height) 
            { 
                ...
                { 
                    int pos = ((BtreePage)items[r]).indexOf(key, height);
                    if (pos >= 0) 
                    { 
                        while (--r >= 0) 
                        { 
            ...
            internal static void  memcpyItems(BtreePage dst_pg, int dst_idx, BtreePage src_pg, int src_idx, int len)
            {
                ...
                Array.Copy(src_pg.nChildren, src_idx, dst_pg.nChildren, dst_idx, len);
            }
            ...
            internal void countChildren(int i, int height) 
            {
                nChildren[i] = ((BtreePage)items[i]).totalCount(height);
            }

            internal int totalCount(int height) 
            { 
                if (--height == 0) 
                { 
                    return nItems;
            ...
            internal void insert(BtreeKey key, int i, int height) 
            {
                insert(key, i);
                if (height != 0) 
                {
        ...
        internal class BtreePageOfObject:BtreePage
        {
            ...
            internal override int compare(Key key, int i)
            {
                int oid = Storage.GetOid(data.GetRaw(i));
        ...
        { 
#if USE_GENERICS        
            internal BtreeSelectionEnumerable(RndBtree<K,V> tree, Key from, Key till, IterationOrder order) 
#else
            internal BtreeSelectionEnumerable(RndBtree tree, Key from, Key till, IterationOrder order) 
#endif
            { 
                this.from = from;
                this.till = till;
        ...
        { 
#if USE_GENERICS        
            internal BtreeSelectionEnumerator(RndBtree<K,V> tree, Key from, Key till, IterationOrder order) 
#else
            internal BtreeSelectionEnumerator(RndBtree tree, Key from, Key till, IterationOrder order) 
#endif
            { 
                this.from = from;
                this.till = till;
...
#if USE_GENERICS        
            internal BtreeSelectionEnumerator(RndBtree<K,V> tree, IterationOrder order) 
#else
            internal BtreeSelectionEnumerator(RndBtree tree, IterationOrder order) 
#endif
            { 
                this.order = order;
                this.tree = tree;
...
#if USE_GENERICS
            protected RndBtree<K,V> tree;
#else
            protected RndBtree tree;
#endif
        }
        ...
        { 
#if USE_GENERICS
            internal BtreeDictionarySelectionEnumerator(RndBtree<K,V> tree, Key from, Key till, IterationOrder order) 
#else
            internal BtreeDictionarySelectionEnumerator(RndBtree tree, Key from, Key till, IterationOrder order) 
#endif
                : base(tree, from, till, order)
            {}

#if USE_GENERICS
            internal BtreeDictionarySelectionEnumerator(RndBtree<K,V> tree, IterationOrder order) 
#else
            internal BtreeDictionarySelectionEnumerator(RndBtree tree, IterationOrder order) 
#endif
                : base(tree, order)
            {}
        ...
        { 
#if USE_GENERICS
            internal BtreeDictionaryStartFromEnumerator(RndBtree<K,V> tree, int start, IterationOrder order) 
#else
            internal BtreeDictionaryStartFromEnumerator(RndBtree tree, int start, IterationOrder order) 
#endif
                : base(tree, order)
            {
                this.start = start;
            ...
            public override void Reset() 
            {
                sp = 0;
                counter = tree.updateCounter;
                if (tree.height == 0 || start >= tree.nElems)
                {
                    return;
                }
                BtreePage page = tree.root;
